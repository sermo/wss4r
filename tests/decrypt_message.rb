...
class DecryptMessage
   ...
   def encrypt()
      ...
      cipherValue = REXML::XPath.first(@document, "/env:Envelope/env:Body/xenc:EncryptedData/xenc:CipherData/xenc:CipherValue")
   ...
   def encrypt_sessionkey()
      #cipherKey = REXML::XPath.first(@document, "/soap:Envelope/soap:Header/wsse:Security/xenc:EncryptedKey/xenc:CipherData/xenc:CipherValue").text()
      cipherKey = REXML::XPath.match(@document, "//xenc:CipherValue")[0].text()
      puts(cipherKey)
   ...
   def extract_key_iv_text(document, key_filename, iv_filename, text_filename)
      ...
      element = REXML::XPath.first(document, "/env:Envelope/env:Body/xenc:EncryptedData/xenc:CipherData/xenc:CipherValue").text()
...
doc = REXML::Document.new(File.read("../xml/wse-response.xml"))
