...
class Signature
	...
	def unprocess(signature)
		@signature_value = REXML::XPath.first(signature, "ds:SignatureValue", {"ds" => Namespaces::DS}).text().gsub("\n","")
		key_info = REXML::XPath.first(signature, "ds:KeyInfo", {"ds" => Namespaces::DS})
	...
	def verify_signature()
		signed_info = REXML::XPath.first(@signature, "ds:SignedInfo", {"ds" => Namespaces::DS})
		inclusive_namespaces = REXML::XPath.first(signed_info, "ds:CanonicalizationMethod/InclusiveNamespaces", {"ds" => Namespaces::DS})
