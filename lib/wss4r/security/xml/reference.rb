module WSS4R
  module Security
	...
	def initialize(element, prefix_list = nil)
          ...
          elements = REXML::XPath.match(element, "Transforms/Transform", {"ds:" => Namespaces::DS})
          #element.each_element("ds:Transforms/ds:Transform"){|e|
          elements.each{|e|
            @transforms.push(e.attribute("Algorithm"))
          }
          elements = REXML::XPath.match(element, "ds:DigestMethod", {"ds" => Namespaces::DS})
          #element.each_element("ds:DigestMethod"){|e|
          elements.each{|e|
            @digest_algorithm = e.attribute("Algorithm")
          }
          elements = REXML::XPath.match(element, "ds:DigestValue", {"ds" => Namespaces::DS})
          #element.each_element("ds:DigestValue"){|e|
          elements.each{|e|
            @digest_value = e.text().strip()
          }
          @ref_element = REXML::XPath.first(element.document, "//*[@wsu:Id='"+@uri+"']")
	end
